-- XP1 community post reward preparation
...
begin
  if exists (
    ...
    raise exception
      'Community posts contain incomplete reward identity';
  end if;
  ...
  if exists (
    ...
    raise exception
      'Community posts contain missing profile identities';
  end if;
  ...
  if exists (
    ...
    raise exception
      'Community posts contain invalid text values';
  end if;

  if exists (
    select 1
    from public.community_posts
    where category not in ('General', 'Intros', 'Wins')
  ) then
    raise exception
      'Community posts contain invalid category values';
  end if;
end
$migration_preflight$;
...
alter table public.community_posts
  drop constraint if exists community_posts_handle_not_blank,
  drop constraint if exists community_posts_body_not_blank,
  drop constraint if exists community_posts_body_length_check,
  drop constraint if exists community_posts_category_whitelist;

alter table public.community_posts
  add constraint community_posts_handle_not_blank
    check (btrim(handle) <> ''),
  ...
  add constraint community_posts_category_whitelist
    check (category in ('General', 'Intros', 'Wins'));


create or replace function
  public.enforce_handle_on_insert()
returns trigger
language plpgsql
security definer
...
create or replace function
  public.protect_community_post_reward_identity()
returns trigger
language plpgsql
...
begin
  ...
  then
    if session_user = 'postgres'
       and coalesce(
         current_setting(
           'app.bypass_community_post_reward_identity',
           true
         ),
         'false'
       ) = 'true'
    ...
    raise exception
      'Community post reward identity fields are immutable';
  end if;
...
drop trigger if exists protect_community_post_reward_identity_trg
on public.community_posts;

create trigger
  protect_community_post_reward_identity_trg
before update of
  id,
  user_id,
  handle,
...
begin
  ...
  if v_category not in ('General', 'Intros', 'Wins') then
    raise exception
